...
from lh_pplsd.layers import (
    PositionEmbedding,
    MultiHeadAttention,
    MSDeformableAttention,
)
from lh_pplsd.layers.utils import _get_clones
from lh_pplsd.layers.param_init import (
    linear_init,
    constant_init,
    xavier_uniform_init,
    normal_init,
...
class DeformableTransformerDecoderLayer(nn.Layer):
    ...
    def forward(
        ...
        Args:
            tgt (Tensor): [B, query_length, C]
            reference_points (Tensor): [B, query_length, n_levels, 2]
            memory (Tensor): [B, value_length, C]
            memory_spatial_shapes (Tensor): [n_levels, 2], [(H_0, W_0), (H_1, W_1), ..., (H_{L-1}, W_{L-1})]
            memory_mask (Tensor): [B, value_length], True for non-padding elements, False for padding elements
        ...
        tgt2 = self.cross_attn(
            q, reference_points, memory, memory_spatial_shapes, memory_mask
        )
