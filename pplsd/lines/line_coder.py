...
class CenterLineJunctionCoder:
    ...
    def decode(
        ...
        with paddle.no_grad():
            ...
            for batch_id in range(B):
                ...
                pred_junc, pred_junc_score = self._calc_junction(
                    jmap[batch_id],
                    joff[batch_id],
                    thresh=junc_score_thresh,
                    top_K=junc_max_num,
                )
                pred_line, pred_line_score = self._calc_line(
                    lmap[batch_id],
                    loff[batch_id],
                    thresh=line_score_thresh,
                    top_K=line_max_num,
                )
    ...
    def _calc_junction(self, jmap, joff, thresh, top_K):
        ...
        juncs = (
            paddle.stack([xs, ys], axis=-1)
            + 0.5
            + joff[indices].reshape([-1, 2])
        )
    ...
    def _calc_line(self, lmap, loff, thresh, top_K):
        ...
        lines = center[:, None] + 0.5 + loff[indices].reshape([-1, 2, 2])
