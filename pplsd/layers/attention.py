...
from pplsd.layers.param_init import xavier_uniform_init, constant_init
...
@manager.ATTENTIONS.add_component
class DirectionalMSDeformableAttention(nn.Layer):
    """
    Directional Multi-Scale Deformable Attention Module
    """
    ...
    def __init__(
        ...
        self.num_levels = num_levels
        self.num_points = num_points
        self.total_directions = num_heads
        self.total_points = num_heads * num_levels * num_points
        ...
        self.sampling_directions = nn.Linear(
            embed_dim,
            self.total_directions * 2,
            weight_attr=ParamAttr(learning_rate=lr_mult),
            bias_attr=ParamAttr(learning_rate=lr_mult),
        )
        self.sampling_lengths = nn.Linear(
            embed_dim,
            self.total_points,
            weight_attr=ParamAttr(learning_rate=lr_mult),
            bias_attr=ParamAttr(learning_rate=lr_mult),
    ...
    def _reset_parameters(self):
        """reset parameters"""
        # sampling_offsets
        constant_init(self.sampling_directions.weight)
        constant_init(self.sampling_lengths.weight)
        thetas = paddle.arange(self.num_heads, dtype=paddle.float32) * (
            2.0 * math.pi / self.num_heads
        )
        grid_init = paddle.stack([thetas.cos(), thetas.sin()], axis=-1)
        grid_init = grid_init / grid_init.abs().max(-1, keepdim=True)
        self.sampling_directions.bias.set_value(grid_init.flatten())

        scaling = paddle.arange(1, self.num_points + 1, dtype=paddle.float32)
        scaling = scaling.reshape([1, 1, -1]).tile(
            [self.num_heads, self.num_levels, 1]
        )
        self.sampling_lengths.bias.set_value(scaling.flatten())
    ...
    def forward(
        ...
        sampling_directions = self.sampling_directions(query).reshape(
            [B, Len_q, self.num_heads, 1, 1, 2]
        )
        sampling_lengths = self.sampling_lengths(query).reshape(
            [B, Len_q, self.num_heads, self.num_levels, self.num_points, 1]
        )
        sampling_offsets = sampling_directions * sampling_lengths
        attention_weights = self.attention_weights(query).reshape(
            [B, Len_q, self.num_heads, self.num_levels * self.num_points]
        )
        attention_weights = F.softmax(attention_weights, -1).reshape(
